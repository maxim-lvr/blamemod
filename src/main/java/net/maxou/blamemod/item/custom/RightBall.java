...
public class RightBall extends Item {
    public RightBall(Properties pProperties) {
        super(pProperties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (!pLevel.isClientSide && pPlayer instanceof ServerPlayer serverPlayer) {
            ...
            serverPlayer.getServer().getCommands().performPrefixedCommand(
                    source, "gravity set_base_direction right " + pPlayer.getName().getString()
            );
            for (int i = 0; i < 9; i++){
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        pTooltipComponents.add(Component.translatable("tooltip.blamemod.right_ball.tooltip"));
